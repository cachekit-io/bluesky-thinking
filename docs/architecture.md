...
`lang_mix`'s `langs` map holds only real language tokens (a post-declared
BCP-47 primary subtag, or `und`) — never a synthetic bucket. Once a window's
...
| **Edge → Cloudflare Workers free plan** | 100k requests/day, 10 ms CPU/invocation — cached analytics reads are single-digit ms. Static dashboard via Workers Assets (free). Cron triggers included (used for history capture; formerly also the Render keep-alive). **Proven live by this spike**: `lab-735-skyline-spike.raywalker.workers.dev` (180 KiB gzipped upload, 2 ms startup, well under the 3 MB compressed script limit). |
